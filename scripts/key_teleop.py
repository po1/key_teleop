...
class SimpleKeyTeleop():
    def __init__(self, interface):
        ...
        self.forward_rate = rospy.get_param('~forward_rate', 0.8)
        self.backward_rate = rospy.get_param('~backward_rate', 0.5)
        self.rotation_rate = rospy.get_param('~rotation_rate', 1.0)
        self.last_pressed = {}
        self.angular = 0
        self.linear = 0
    ...
    def _set_velocity(self):
        now = rospy.get_time()
        keys = []
        for a in self.last_pressed:
            if now - self.last_pressed[a] < 0.4:
                keys.append(a)
        linear = 0.0
        angular = 0.0
        for k in keys:
            ...
            angular += a
        if linear > 0:
            linear = linear * self.forward_rate
        else:
            linear = linear * self.backward_rate
        angular = angular * self.rotation_rate
        self._angular = angular
        self._linear = linear
    ...
    def _key_pressed(self, keycode):
        ...
        if keycode == ord('q'):
            ...
            self.last_pressed[keycode] = now

    def _publish(self):
        self.interface.clear()
        self.interface.write_line(2, 'Linear: %d, Angular: %d' % (self.linear, self.angular))
        self.interface.write_line(5, 'Use arrow keys to move, space to stop, q to exit.')
        self.interface.refresh()

        twist = self._get_twist(self.linear, self.angular)
        self._pub_cmd.publish(twist)
